...
export default function Home() {
  ...
  const mouseClick: React.MouseEventHandler<HTMLCanvasElement> = (event) => {
    if (actionTracker.selectedTool.actionType !== DrillActions.draw) return;
  ...
  return (
    <main className="max-w-[992px] mx-auto">
      <div className="flex justify-center items-center gap-4 flex-wrap mt-4">
        {
          drillMaps.map((map, index) => <Tooltip text={map.label} key={index}>
            <div className="group bg-gray-100 p-4 rounded-md h-24 w-24 m-auto cursor-pointer" onClick={() => setActionTracker((prevAction) => ({ ...prevAction, selectedMap: map.svgImagePath }))}>
      ...
      <ToolSelection onToolChange={onChangeTool} selectedTool={actionTracker.selectedTool} />
    </main>
  );
}
